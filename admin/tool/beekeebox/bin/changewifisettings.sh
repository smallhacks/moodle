...
SSIDHIDDEN="$(grep '^ssidhiddenstate\b' $FILE | cut -d= -f2)"
...
#
# Password setting.
# Validate password length and allowed chars. Replace it with 'beekeebox' if invalid.
...
[[ $NEWPASSWORD =~ ^[\ -z\{\|\}\~]{8,63}$ ]] || NEWPASSWORD="beekeebox"
...
#
# SSID setting.
# Validate new SSID. Replace it with 'BeekeeBox' if invalid.
...
[[ $NEWSSID =~ ^([0-9a-fA-F]{2}){1,32}$ ]] || NEWSSID="4d6f6f646c65426f78" # "BeekeeBox"
...
if [ "$PASSWORDPROTECTED" -eq 1 ]; then
    ...
    if [[ "$ISCURRENTLYPROTECTED" == true ]]; then
        ...
        sed -i "/^wpa_passphrase=/c\#wpa_passphrase=beekeebox" "$CONFIGFILE"
        sed -i "/^wpa=/c\#wpa=2" "$CONFIGFILE"
        sed -i "/^wpa_key_mgmt=/c\#wpa_key_mgmt=WPA-PSK" "$CONFIGFILE"
        sed -i "/^rsn_pairwise=/c\#rsn_pairwise=CCMP" "$CONFIGFILE"
    fi
