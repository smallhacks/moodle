 ...
 */
function local_boostnavigation_beekee_extend_navigation(global_navigation $navigation) {
    global $CFG, $PAGE, $COURSE;

    // Fetch config.
    $config = get_config('local_boostnavigation_beekee');
        ...
        if ($mycoursesnode) {
            ...
            foreach ($mycourseschildrennodeskeys as $k) {
                ...
                if ($CFG->navshowmycoursecategories) {
                    ...
                    $allchildrennodes = local_boostnavigation_beekee_get_all_childrenkeys($mycoursesnode->get($k));
    ...
    if (isset($config->collapsemycoursesnode) && $config->collapsemycoursesnode == true
        ...
        if ($mycoursesnode) {
            ...
            $userprefmycoursesnode = get_user_preferences('local_boostnavigation_beekee-collapse_mycoursesnode',
        ...
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID) {
            ...
            if (count($allsectionnodes) > 0) {
                ...
                $coursesectionsnode = navigation_node::create(get_string('sections', 'moodle'),
                        ...
                        null,
                        'localboostnavigation_beekeecoursesections',
                        null);
                ...
                if ($config->collapsecoursesectionscoursenode == true) {
                    ...
                    if ($coursesectionsnode) {
                        // Remember the collapsible node for JavaScript.
                        $collapsenodesforjs[] = 'localboostnavigation_beekeecoursesections';
                        ...
                        // nav drawer, so we accept this abuse.
                        $userprefcoursesectionsnode = get_user_preferences('local_boostnavigation_beekee-collapse_'.
                                'localboostnavigation_beekeecoursesectionsnode', $config->collapsecoursesectionscoursenodedefault);
                        if ($userprefcoursesectionsnode == 1) {
                            ...
                            foreach ($coursehomenodechildrennodeskeys as $k) {
                                ...
                                if ($node->action) {
                                    ...
                                    if ($urlpath == '/course/view.php' && $node->key != 'localboostnavigation_beekeecoursesections') {
        ...
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID) {
            ...
            if (!empty($modfullnames)) {
                ...
                $activitiesnode = navigation_node::create(get_string('activities', 'moodle'),
                        ...
                        global_navigation::TYPE_CUSTOM,
                        null,
                        'localboostnavigation_beekeeactivities',
                        null);
                    ...
                    if ($modname === 'resources') {
                        ...
                        if ($config->insertresourcescoursenode == false) {
                            $activitynode = navigation_node::create($modfullname,
                                    ...
                                    null,
                                    'localboostnavigation_beekeeactivity'.$modname,
                                    new pix_icon('resources', '', 'local_boostnavigation_beekee'));
                        ...
                        $activitynode = navigation_node::create($modfullname,
                                ...
                                null,
                                'localboostnavigation_beekeeactivity'.$modname,
                                new pix_icon('activities', '', 'local_boostnavigation_beekee'));
                        // Add the activity course node to the coursehome node.
                        $coursehomenode->add_node($activitynode);
                        // Remember the activity course node's key for collapsing it later.
                        $activitiesnodechildrennodeskeys[] = $activitynode->key;
                ...
                if ($config->collapseactivitiescoursenode == true) {
                    ...
                    if ($activitiesnode) {
                        // Remember the collapsible node for JavaScript.
                        $collapsenodesforjs[] = 'localboostnavigation_beekeeactivities';
                        ...
                        // existing section nodes.
                        $userprefactivitiesnode = get_user_preferences('local_boostnavigation_beekee-collapse_'.
                                'localboostnavigation_beekeeactivitiesnode', $config->collapseactivitiescoursenodedefault);
        ...
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID) {
            ...
            if (array_key_exists('resources', $modfullnames)) {
                ...
                $resourcesnode = navigation_node::create(get_string('resources'),
                        ...
                        global_navigation::TYPE_ACTIVITY,
                        null,
                        'localboostnavigation_beekeeresources',
                        new pix_icon('resources', '', 'local_boostnavigation_beekee'));
                // Add the activities node to the end of the course navigation.
    ...
    if (isset($config->insertcustomnodesusers) && !empty($config->insertcustomnodesusers)) {
        // If yes, do it.
        $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustomnodesusers, $navigation,
                'localboostnavigation_beekeecustomrootusers', true, $config->collapsecustomnodesusers,
    ...
    if (isset($config->insertcustomnodesadmins) && !empty($config->insertcustomnodesadmins) && is_siteadmin()) {
        // If yes, do it.
        $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustomnodesadmins, $navigation,
                'localboostnavigation_beekeecustomrootadmins', true, $config->collapsecustomnodesadmins,
    ...
    if (isset($config->insertcustomcoursenodesusers) && !empty($config->insertcustomcoursenodesusers)) {
        ...
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID) {
            // If yes, do it.
            $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustomcoursenodesusers, $coursehomenode,
                    'localboostnavigation_beekeecustomcourseusers', false, $config->collapsecustomcoursenodesusers,
    ...
    if (isset($config->insertcustomcoursenodesadmins) && !empty($config->insertcustomcoursenodesadmins) && is_siteadmin()) {
        ...
        if ($PAGE->context->get_course_context(false) == true && $COURSE->id != SITEID) {
            // If yes, do it.
            $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustomcoursenodesadmins, $coursehomenode,
                    'localboostnavigation_beekeecustomcourseadmins', false, $config->collapsecustomcoursenodesadmins,
    ...
    if (isset($config->insertcustombottomnodesusers) && !empty($config->insertcustombottomnodesusers)) {
        // If yes, do it.
        $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustombottomnodesusers, $navigation,
                'localboostnavigation_beekeecustombottomusers', true, $config->collapsecustombottomnodesusers,
    ...
    if (isset($config->insertcustombottomnodesadmins) && !empty($config->insertcustombottomnodesadmins) && is_siteadmin()) {
        // If yes, do it.
        $customnodesret = local_boostnavigation_beekee_build_custom_nodes($config->insertcustombottomnodesadmins, $navigation,
                'localboostnavigation_beekeecustombottomadmins', true, $config->collapsecustombottomnodesadmins,
    ...
    if (!empty($collapsenodesforjs)) {
        ...
        $PAGE->requires->js_call_amd('local_boostnavigation_beekee/collapsenavdrawernodes', 'init', [$collapsenodesforjs]);
        // Allow updating the necessary user preferences via Ajax.
        foreach ($collapsenodesforjs as $node) {
            user_preference_allow_ajax_update('local_boostnavigation_beekee-collapse_'.$node.'node', PARAM_BOOL);
        }
    }
}
 ...
 */
function local_boostnavigation_beekee_extend_navigation_course(navigation_node $navigation) {
    global $PAGE, $COURSE;
 ...
 */
function local_boostnavigation_beekee_get_fontawesome_icon_map() {
    return [
            'local_boostnavigation_beekee:customnode' => 'fa-square local-boostnavigation_beekee-fa-sm',
            'local_boostnavigation_beekee:resources' => 'fa-archive',
            'local_boostnavigation_beekee:activities' => 'fa-share-alt',
    ];
}
